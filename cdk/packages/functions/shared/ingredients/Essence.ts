...
import type { ModEntry, OmenType } from "../types";
...
export class Essence implements CraftingIngredient {
  ...
  apply(item: CraftedItem, ctx: CraftContext) {
    ...
    if (this.tier === "perfect") {
      const removed = next.removeRandomAffix(ctx, this.omen);
      next = removed.item.addMod(this.guaranteedMod);
      return craftResult(next, { [this.id]: 1 }, [
        {
          type: "currency",
          message: this.displayName,
          details: { tier: this.tier, removed: removed.removed?.modId ?? null, guaranteed: this.guaranteedMod.modId, omen: this.omen },
        },
      ]);
    }

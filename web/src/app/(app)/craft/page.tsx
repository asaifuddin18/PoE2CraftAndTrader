...
function ExactTrace({ trace,div }:{ trace:SimulationTrace;div:number }) { const mods=itemMods(trace.finalItem); return <details className="border rounded" style={{ borderColor:"var(--border)",background:"var(--bg-elevated)" }}><summary className="cursor-pointer px-3 py-2 grid grid-cols-[90px_1fr_110px] gap-3 text-xs"><strong>Item {trace.id}</strong><span>{trace.finalItem.rarity} · {mods.length} affixes · {trace.steps.length} decisions</span><span className="text-right">{formatCurrency(trace.spend,div)}</span></summary><div className="border-t px-3 py-3" style={{ borderColor:"var(--border)" }}><p className="text-xs font-semibold mb-2">Crafting steps</p>{trace.steps.length?trace.steps.map((step,index)=><TraceStep key={`${trace.id}-${index}`} index={index} step={step} previous={index?trace.steps[index-1].itemAfter:null}/>):<p className="text-xs" style={{ color:"var(--text-disabled)" }}>No crafting actions were needed.</p>}<div className="mt-4 pt-3 border-t" style={{ borderColor:"var(--border)" }}><p className="text-xs font-semibold mb-1">Final item</p><TraceMods item={trace.finalItem}/></div></div></details>; }
function TraceStep({ index,step,previous }:{ index:number;step:SimulationTrace["steps"][number];previous:SimulationTraceItem|null }) { const before=itemMods(previous); const after=itemMods(step.itemAfter); const beforeKeys=new Set(before.map(modKey)); const afterKeys=new Set(after.map(modKey)); const removed=before.filter(mod=>!afterKeys.has(modKey(mod))); return <div className="grid grid-cols-[24px_minmax(180px,0.65fr)_minmax(260px,1.35fr)_auto] gap-3 py-2.5 border-b text-xs" style={{ borderColor:"var(--border)" }}><span style={{ color:"var(--text-disabled)" }}>{index+1}</span><div><p className="font-medium">{step.action}</p>{step.events.length>1&&<p className="mt-0.5 text-[10px]" style={{ color:"var(--text-disabled)" }}>{step.events.map(event=>event.message).join(" → ")}</p>}{removed.length>0&&<p className="mt-1 text-[10px]" style={{ color:"var(--status-negative)" }}>Removed: {removed.map(modLabel).join(" · ")}</p>}</div><div className="flex flex-wrap gap-x-3 gap-y-1">{after.length?after.map(mod=><span key={modKey(mod)} style={{ color:beforeKeys.has(modKey(mod))?"var(--text-disabled)":"var(--status-positive)" }}>{modLabel(mod)}</span>):<span style={{ color:"var(--text-disabled)" }}>No affixes</span>}</div><span className="text-right" style={{ color:"var(--text-disabled)" }}>{Object.entries(step.cost).map(([currency,count])=>`${count} ${currency.replaceAll("_"," ")}`).join(" + ")}</span></div>; }
